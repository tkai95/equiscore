...
export function PoliciesView({ organisationSlug }: { organisationSlug: string }) {
  ...
  return (
    <PageLayout width="wide">
      <PageHeader title="Policies" description="Approved company criteria used to evaluate assessment snapshots." />
      <Card>
        {isLoading ? (
          ...
          <WorkspaceTable columns={['Policy', 'Type', 'Status', 'Latest version', 'Owner', 'Updated']}>
            {policies.map((policy) => (
              <tr key={policy.id}>
                <Cell>
                  <p className="font-medium">{policy.name}</p>
                  <p className="text-xs text-content-muted">{policy.versionCount} version{policy.versionCount === 1 ? '' : 's'}</p>
                </Cell>
                <Cell>{label(policy.assessmentType)}</Cell>
                <Cell>
                  <StatusPill status={policy.status === 'active' ? 'success' : 'neutral'} label={label(policy.status)} />
                </Cell>
                <Cell muted>{policy.latestVersion ? `v${policy.latestVersion.versionNumber} · ${label(policy.latestVersion.status)}` : 'None'}</Cell>
