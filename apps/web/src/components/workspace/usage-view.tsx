...
export function UsageView({ organisationSlug }: { organisationSlug: string }) {
  ...
  return (
    <PageLayout width="wide">
      <PageHeader title="Usage" description="Append-only assessment-credit ledger for delivered assessment value." />
      <Card>
        {isLoading ? (
          ...
          <WorkspaceTable columns={['Event', 'Applicant', 'Case', 'Quantity', 'Billing', 'Occurred']}>
            {events.map((event) => (
              <tr key={event.id}>
                <Cell>
                  <p className="font-medium">{label(event.eventType)}</p>
                  <p className="text-xs text-content-muted">{label(event.source)}</p>
                </Cell>
                <Cell muted>{event.applicant?.name ?? 'None'}</Cell>
                <Cell muted>{event.assessmentCase?.reference ?? event.assessmentCase?.id ?? 'None'}</Cell>
                <Cell>{event.quantity} {label(event.unit)}</Cell>
                <Cell>
